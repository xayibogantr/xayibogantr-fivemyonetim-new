...
Here's a list of common problems people have run into and how to solve them. If you are having a problem that isn't listed here, check the [readme](https://github.com/xayibogantr/xayibogantr/blob/djs/readme.md) and if there's still nothing submit an [issue](https://github.com/xayibogantr/xayibogantr/issues) if it doesn't already exist there.
...
**needs** `Presence Intent` and `Server Members Intent` from the discord bot [settings](https://discord.com/developers/applications) to be enabled for basic functionality. Without these the bot is unable to check if a user is in your discord server or if they have the required roles to join the server if using the whitelist feature.
...
![intents](/xayibogantr/images/intents.png)


### Can't find yarn dependency

This resource requires FiveM's yarn module from [cfx-server-data](https://github.com/citizenfx/cfx-server-data) under `(resources/[system]/[builders]/yarn)` so either add those resources to your resource folder or just put the yarn one in your resources and xayibogantr will enable it for itself so you don't need to include it in your cfg.
...
**This was patched in version 7.0.0 of and FiveM artifact 4980+. Update to these or later to make this go away.**
...
When trying to start you get an error that says `Could not find dependency /server:4890 for resource`. This means you aren't running [FiveM artifacts](https://runtime.fivem.net/artifacts/fivem/build_server_windows/master/) version 4890 or later. updating to a version number higher than 4890 will fix this error and allow the resource to start.
...
You really shouldn't use a custom chat but rather theme the default but IF you did.. you can change the functionality of the chat in by modifying the function `chatMessage` in `utils.js` as all messages in game are forwarded through therre
